...
- **AI: Extended instructions, events & programs** — Added 14 new AI instructions: `TargetEntity`, `MoveToTarget`, `LookAtTarget`, `AimAtTarget`, `PrimaryAttack`, `SecondaryAttack`, `MoveToCover`, `Crouch`, `StandUp`, `SetMoveMode`, `PlayAnimation`, `EquipWeapon`, `UnequipWeapon`, `ChatMessageContains`. Added 9 new events: `OnPlayerChat`, `OnNPCChat`, `OnAllyAttacked`, `OnEnemyKilled`, `OnLowHealth`, `OnStuck`, `OnPlayerInRange`, `OnEnemyInRange`, `OnFriendlyInRange`. NPC now has Health/Team/IsCrouching/WeaponEquipped state, move modes (walk/run/sprint), animation override timer, cover-finding logic, entity targeting via network ID, chat message buffer. Server relays player chat to NPCs and passes weapon damage to `OnAttacked`. Expanded `FunkyBehavior` program (77 steps) to exercise all instructions and events. Added `AIStep` factory methods (`MoveToTargetAt`, `Attack`, `SecondaryAttack`, `SetMode`, `PlayAnim`, `ChatContains`).
