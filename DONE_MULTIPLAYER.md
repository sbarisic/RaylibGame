...
- **Spawn point selection spawns inside ground** — `FindSpawnPoints` required only any solid block with 2 air blocks above. Now requires a grass block on a flat 3×3 surface (±1 height tolerance) with 3 air blocks of clearance. Scan starts 1 block in from world edges to allow neighbor checks.
- **Render distance culling** — `ChunkMap.Draw()` and `DrawTransparent()` iterated all loaded chunks regardless of distance. Added `RenderDistanceBlocks` field (default 160 blocks / 10 chunks) with squared-distance check from camera to chunk center, skipping distant chunks before frustum testing. Significantly reduces draw calls and transparent face sorting overhead.
- **Cross-chunk skylight propagation** — Skylight stopped at chunk boundaries because `ComputeSkylight` only seeded from sky-exposed blocks, ignoring pre-existing skylight values written into boundary blocks by earlier parallel phases. Added `SeedFromBoundarySkylight` pass after sky-exposure seeding that scans all 6 chunk faces for non-opaque blocks with skylight > 1 and enqueues them for further inward propagation. Block light was already correct via `CollectNeighborLightSources`.
- **Prediction/reconciliation optimization** — Three improvements for smoother client prediction at higher latency: (1) Increased `CorrectionThreshold` from 0.01 to 0.1 world units to reduce unnecessary reconciliations from floating-point drift. (2) Added velocity divergence check (`VelocityCorrectionThreshold = 0.5`) to catch future drift early even when position matches. (3) Added visual error smoothing in `MultiplayerGameState`: corrections below `SnapThreshold` (3.0 units) accumulate a `_correctionSmoothOffset` blended toward zero per frame (`CorrectionSmoothRate = 12`), applied to the render camera only. Large corrections snap immediately. Pre-allocated `ClientInputBuffer._replayList` to eliminate per-reconciliation GC allocation.
- **Multiplayer unit tests** — 64 tests in `UnitTest\NetworkTests.cs` covering: packet serialization round-trips (Connect, ConnectAccept, ConnectReject, PlayerSnapshot, WorldSnapshot, InputState), InputStatePacket `PackKeys`/`UnpackKeys` bitmask fidelity, `ClientPrediction` thresholds and reconciliation detection (position error, velocity divergence, old/duplicate tick rejection, buffer wraparound), `ClientInputBuffer` record/retrieve/range/clear/wraparound, `ReliableChannel` wrap/unwrap/ACK piggybacking/retransmission/duplicate detection/bitfield ACK, and `SnapshotBuffer` interpolation math (midpoint, clamp-before-oldest, clamp-after-newest, reset, wraparound). All 90 tests pass (64 new + 26 existing).
- **Frustum-aware lighting and render distance** — Reduced `RenderDistanceBlocks` from 160 to 128 (8 chunks). `SetPlacedBlock` now skips lighting recalculation for chunks outside render distance, marking them `NeedsRelighting` instead. `ChunkMap.Draw()` batch-relights deferred chunks when they enter render distance using the 8-phase parallel system. Eliminates wasted lighting computation on invisible chunks (especially impactful for remote block changes in multiplayer).
