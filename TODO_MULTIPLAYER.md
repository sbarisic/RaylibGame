...
- [ ] **ServerLoop: Split into partial class files** — `ServerLoop.cs` (773 lines) has grown too large. Split into partial class files in `Engine/Server/` folder by responsibility: `ServerLoop.cs` (core lifecycle, tick loop, fields/constants), `ServerLoop.Connections.cs` (connect/disconnect handlers, player management), `ServerLoop.Packets.cs` (packet dispatch, block request handlers), `ServerLoop.Combat.cs` (weapon fire handling, player raycasting, respawns), `ServerLoop.Broadcasting.cs` (player snapshots, entity snapshots, block changes, time sync), `ServerLoop.Entities.cs` (entity spawning, animation state mapping). Follow established patterns from Player/Chunk partial class splits. Move everything else server related to that folder. **[CPX: 3]**
...
- Now that singleplayer is removed, what parts of code are redundant or unused? Handle these, add singleplayer functionality not yet implemented in multiplayer to TODO_MULTIPLAYER, remove other unused code
...
- **Packet fragmentation** — Created `PacketFragmenter` class in `VoxelgineEngine/Engine/Net/PacketFragmenter.cs` implementing transport-layer fragmentation for large reliable packets. Fragment wire format: `[0xFE][groupId:2][index:1][total:1][payload...]` — the 0xFE marker byte does not conflict with `PacketType` values (0x01–0x81) or `PacketBatcher.BatchMarker` (0xFF). **Send side:** `NeedsFragmentation(byte[])` checks if serialized data exceeds `MaxFragmentPayload` (MTU - ReliableChannel.HeaderSize - FragmentHeaderSize = 1186 bytes). `Split(byte[])` assigns a monotonically increasing group ID (ushort, skipping 0), divides the data into fragments (up to 255, max ~302 KB total), and prepends the 5-byte fragment header to each. Each fragment is sent as an individually reliable packet through `ReliableChannel` (gets its own sequence number for retransmission). **Receive side:** `HandleReceived(byte[], float)` checks the fragment marker; non-fragments pass through unchanged. Fragments are buffered in a `Dictionary<ushort, FragmentGroup>` keyed by group ID. `FragmentGroup` tracks a `byte[][]` array indexed by fragment index, counts received fragments, and records `LastFragmentTime` for stale cleanup. When all fragments for a group arrive (`IsComplete`), `Assemble()` concatenates payloads in order and returns the original packet data. Duplicate fragments (from reliable retransmission) silently overwrite without incrementing the count. `CleanupStaleGroups(float)` removes incomplete groups after 10-second timeout (`StaleGroupTimeout`). **Integration:** Added `PacketFragmenter _fragmenter` field and `Fragmenter` property to `NetConnection`. Modified `UnwrapPacket()` to call `_fragmenter.HandleReceived()` after `ReliableChannel.Unwrap()` — returns null for partial fragments, reassembled data when complete. Added `QueuePacket(Packet, bool, float)` method for server batched sends — serializes, checks fragmentation, splits if needed, wraps each fragment through channel, queues each. Added `SendImmediate(Packet, bool, float, UdpTransport)` method for client direct sends — same logic but sends through transport immediately. Added `CleanupStaleFragments(float)` called per-tick. Updated `NetServer.QueuePacket()` to delegate to `connection.QueuePacket()`, `SendDirect()` to `connection.SendImmediate()`, added stale cleanup in tick loop. Updated `NetClient.SendInternal()` to use `connection.SendImmediate()`, added stale cleanup in tick. Retransmissions of fragment packets handled automatically by `ReliableChannel` — fragment headers are preserved in `PendingPacket.PacketData`. Works alongside existing application-layer `WorldTransferManager` fragmentation (1024-byte `WorldDataPacket`s) as defense-in-depth. Build verified.
