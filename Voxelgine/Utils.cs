		...
		public static bool Raycast2(Vector3 Origin, Vector3 Direction, float Length, float CastScale, Raycast2CallbackFunc Callback) {
			...
			while (true) {
				...
				if (Callback(hitPos, face)) {
					if (IsRecording) {
						RaycastDrawList.Add(new Tuple<Vector3, Vector3, Color>(Origin / CastScale, hitPos, Raylib_cs.Color.Red));
					}
			...
			if (IsRecording) {
				RaycastDrawList.Add(new Tuple<Vector3, Vector3, Color>(Origin / CastScale, (Origin + (Direction * Length)) / CastScale, Raylib_cs.Color.Blue));
			}
			return false;
		}
